...
//        String userName = "张三";
    ...
    @Test
   public void test02(){
       // 通过of方法 of方法是不支持null的
       final Optional<String> op1 = Optional.of("zhangsan");
//        final Optional<Object> op2 = Optional.of(null);

        // 第二种方式通过 ofNullable方法 支持null
        final Optional<String> op3 = Optional.ofNullable("lisi");
        final Optional<Object> op4 = Optional.ofNullable(null);

        // 第三种方式，通过empty方法直接创建一个空的Optional对象
        final Optional<Object> op5 = Optional.empty();
    }
    ...
    public void test03(){
        final Optional<String> op1 = Optional.of("zhangsan");
        final Optional<String> op2 = Optional.empty();

        //获取Optional中的值
        if (op1.isPresent()) {
            final String s1 = op1.get();
        ...
        final String s3 = op1.orElse("李四");
        System.out.println(s3);

        String s4 = op2.orElse("王五");
        System.out.println(s4);

        final String s5 = op2.orElseGet(() -> {
            return "Hello";
        });
    ...
    public void test04(){
        final Optional<String> op1 = Optional.of("zhangsan");
        final Optional<Object> op2 = Optional.empty();
        // 如果存在值 就做什么
    ...
    public void test05(){
        ...
        final Optional<Person> op = Optional.of(p);
        final String name = getNameForOptional(op);
    ...
    public String getNameForOptional(Optional<Person> op){
        if (op.isPresent()){
            final String msg = //op.map(p -> p.getName())
