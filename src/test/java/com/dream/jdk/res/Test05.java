...
public class Test05 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("总的消耗时间：" + (end-start));
    }
}
...
class SumRecursiveTask extends RecursiveTask<Long> {
    ...
    protected Long compute() {
        ...
        if (length <= THRESHOLD) {
            ...
            //数量大于预定的数量，那说明任务还得继续拆分
            long middle = (start+end)/2;
            System.out.println("拆分：左边 " + start + "--> " + middle + ", 右边 " + (middle+1)+ " -->" + end);
            final SumRecursiveTask left = new SumRecursiveTask(start, middle);
            left.fork();
            final SumRecursiveTask right = new SumRecursiveTask(middle + 1, end);
            right.fork();
